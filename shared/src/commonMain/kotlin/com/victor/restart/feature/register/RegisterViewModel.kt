...
class RegisterViewModel (
    ...
    override fun handleAction(action: RegisterAction) {
        when(action){
            ...
            is RegisterAction.Internal.ReceiveRegisterResult -> handleRegisterResult(action)
    ...
    private fun handleRegisterResult(action: RegisterAction.Internal.ReceiveRegisterResult){
        viewModelScope.launch {
            when(action.registerResult){
                is DataState.Error ->{
                    val errMsg =
                        if (action.registerResult.exception.cause is ServerResponseException){
                            getString(Res.string.internal_server_error)
                        }else{
                            action.registerResult.message
                        }
                    updateState {
                        it.copy(
                            isError = true,
                ...
                is DataState.Success -> {
                    ...
                    val registerData = action.registerResult.data
                    updateState {
                        it.copy(
                            isError = false
                        )
                    }
                    val registerResult = registerData
